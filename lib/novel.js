...
async function readNovel(input, chapterIndex = 1) {
    ...
    try {
        ...
        let startLine = chapters[chapterIndex - 1];
        let endLine = chapters[chapterIndex] || content.split('\n').length;

        const linesToRead = content.split('\n').slice(startLine, endLine).join('\n');
        // 现在`linesToRead`包含了从指定章节开始到下一章节（或文件末尾）的所有文本

        // 使用`paginate`函数显示内容
        const pageSize = process.stdout.rows - 4;
        const page = paginate(linesToRead, pageSize);
        page();
